...
use(
    ...
    }, function(req: Request, email, password, done) {
        Service.authToken(Admin, {email}, password, req).then(function (token: string) {
...
use(
    ...
    new LocalStrategy({
        ...
        Service.authToken(User, {email}, password, req).then(function (token: string) {
            done(null, token, {message: LOGIN.SUCCESS});
        }).catch(function (error: ResponseError) {
            done(error);
        });
